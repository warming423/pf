...
class RunProfile:
    ...
    ###将 GPU 监测指标以 JSON 格式附加到原始数据中，返回压缩后的数据
    ...
    ### 接受一个可迭代的 MemoryRecord 对象以及起始和结束时间戳，并返回时间有序的内存分配和释放事件
    ### 一个memory对象可能为：MemoryRecord('scope1', 1, 1, 100, DeviceType.CPU, 0, 0x01, 512, 1024, 2048)  e.ts返回的是该对象的时间值，根据该值过滤事件
    @staticmethod
