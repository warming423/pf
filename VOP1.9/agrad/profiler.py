...
import torch
import agrad
from agrad import DeviceType, ProfilerActivity, ProfilerState
...
class profile(object):
    ...
    Example:
        ...
        >>> with agrad.profiler.profile() as prof:
    ...
    def _prepare_trace(self):
        ...
        if self.kineto_activities:
            agrad._prepare_profiler(self.config(), self.kineto_activities)
        else:
    ...
    def _start_trace(self):
        self.entered = True
        if self.kineto_activities:
            agrad._enable_profiler(self.config(), self.kineto_activities)
        else:
            agrad._enable_profiler_legacy(self.config())
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        ...
        if self.kineto_activities:
            self.kineto_results = agrad._disable_profiler()
            parsed_results = parse_kineto_results(self.kineto_results)
        else:
            records = agrad._disable_profiler_legacy()
    ...
    def __repr__(self):
        if self.function_events is None:
            return '<unfinished agrad.profile>'
...
class emit_nvtx(object):
    ...
    **Double-backward**
    ...
    def __enter__(self):
        ...
        self.entered = True
        torch.cuda.synchronize()
        agrad._enable_profiler_legacy(
            agrad.ProfilerConfig(
                ProfilerState.NVTX,
                self.record_shapes,
                False,
                False,
    ...
    def __exit__(self, exc_type, exc_val, exc_tb):
        if not self.enabled:
            return
        torch.cuda.synchronize()
        agrad._disable_profiler_legacy()
        return False
